/**
 * PR17.5: Personalization Agent - Copy Templates
 * 
 * Persona-based, localized copy for all viral loops
 * Structure: loopType → persona → locale → copy string
 * 
 * Supported:
 * - Loops: tutor_card, progress_reel, study_buddy, parent_pod, tutor_peer, results
 * - Personas: tutor, parent, student
 * - Locales: en, es, fr
 */
 ...
 * 2. English ('en')
 * 3. Generic fallback from fallbackContent.ts
 * 
...
export function getCopyTemplate(
  ...
  if (loopTemplates) {
    const personaTemplates = loopTemplates[persona];
    if (personaTemplates) {
      const localizedCopy = personaTemplates[locale];
      if (localizedCopy) {
  ...
  return getFallbackCopy(loopType, persona);
}
